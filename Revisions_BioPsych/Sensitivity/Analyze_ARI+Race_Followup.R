...
inputPath<-'/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/processedData/follow-up/n141_Nmf24BasesCT_COMBAT_TP2.csv'
...
OutDirRoot <- '/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/FINAL/Revisions_BioPsych/Sensitivity'
covsFormula <- "~s(ScanAgeYears,k=4)+sex+rating+ari_log+race"
...
Final_Sample<-readRDS("/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/processedData/longitudinal/n137_Demo+Psych+DX+QA_20180531.rds")
dataSubj<-dataSubj[which(dataSubj$bblid %in% Final_Sample$bblid),]
...
m <- mclapply(model.formula, function(x) {
  ...
  MODELS <- gam(formula = x, data=dataSubj, method="REML")
  summary <- summary(MODELS)
  residuals <- MODELS$residuals
  missing <- as.numeric(MODELS$na.action)
...
if (is.null(m[[1]]$s.table)) {
  ...
  output<-output[order(output$tval.ari_log),]
