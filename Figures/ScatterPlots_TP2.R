...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(ScanAgeYears,k=4) + sex + rating + ari_log, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
Limbic<-ggplot() +
  ...
  geom_line(data = smooths, aes(x = x, y = smooth), colour = lineColor,size=3) +
  geom_line(data = smooths, aes(x = x, y=lower), linetype="dashed", colour = lineColor, alpha = 0.9, size = 2) +
  geom_line(data = smooths, aes(x = x, y=upper), linetype="dashed",colour = lineColor, alpha = 0.9, size = 2) +
  annotate("text",x = -Inf, y = Inf, hjust = -0.1,vjust = 1,label = p_text, parse=TRUE,size = 8, colour = "black",fontface ="italic" ) +
  theme(legend.position = "none") +
  labs(x = "Irritability (ARI)", y = "CT of Network 18 (a.u.)") +
...
ggsave(file="/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/NMF_Figures/F3_TP2/ScatterPlot_ctTP2_Network18.pdf",device = "pdf", width = 4, height = 5.5)
...
Limbic<-ggplot() +
  ...
  annotate("text",x = -Inf, y = Inf, hjust = -0.1,vjust = 1,label = p_text, parse=TRUE,size = 8, colour = "black",fontface ="italic" ) +
  theme(legend.position = "none") +
  labs(x = "Irritability (ARI)", y = "CT of Network 9 (a.u.)") +
  theme(axis.title=element_text(size=21,face="bold"), axis.text=element_text(size=21), axis.title.x=element_text(color = "black"), axis.title.y=element_text(color = "black"))

### Save Scatterplot ###

ggsave(file="/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/NMF_Figures/F3_TP2/ScatterPlot_ctTP2_Network9.pdf",device = "pdf", width = 4, height = 5.5)
