...
data.NMF<-merge(RDS,CSV, by=c("bblid"))
nmfComponents <- names(data.NMF)[grep("Ct_Nmf24C",names(data.NMF))]

#Define Formulas

NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ TP1_ageAtScan1 + TP2_sex + DeltaQA + TP2_ari_log, list(i = as.name(x))), method="REML", data = data.NMF)
})
