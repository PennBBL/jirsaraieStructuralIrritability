...
inputPath<-'/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/processedData/longitudinal/n137_Nmf9SIGBasesCT_COMBAT_Rate.csv'
subjDataName<-'/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/processedData/longitudinal/n137_Demo+Psych+DX+QA+PRIME_20190713.rds'
OutDirRoot <- '/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/FINAL/Revisions_NPP/Sensitivity'

covsFormula <- "~s(TP1_ageAtScan1,k=4)+TP2_sex+DeltaQA+TP2_ari_log+TP2_PRIME_total"

##############################################################################
################              Adjust Parameters               ################
##############################################################################

inclusionName <- 'DeltaT1Exclude'
subjID <- 'bblid'
pAdjustMethod <- 'fdr'
ncores <- 5
residualMap <- FALSE
...
if (is.null(m[[1]]$s.table)) {
  ...
  output<-output[order(output$tval.TP2_ari_log),]
